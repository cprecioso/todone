...
- `dryRun` (default `false`): Used by the `@todone/plugin-github/create-issues` reporter. When `true`, issue mutations are logged but never sent to GitHub.
...
- `@todone/plugin-github/create-issues`: Reconciles expired TODOs against the open `todone`-labeled issues and creates, updates, or closes issues accordingly, then writes a job summary describing what happened. Honors the `dryRun` plugin option.
